        ...
        const fetchData = async () => {
            try {
                const response = await axios.get(
                    'http://localhost:8000/api/v1/assessment/quiz/b04f63e9-595a-4056-98c5-601a103f4476/?format=json',
                    { timeout: 5000 }
                );
        ...
        const fetchData = async () => {
            try {
                const response = await axios.post(
                    'http://localhost:8000/api/v1/assessment/quizAnswer/' +
                        quiz._id +
                        '/create/',
                    { timeout: 5000 }
                );
    ...
    const sendNewQuestionAnswer = (question, key, value) => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.post(
                    'http://localhost:8000/api/v1/assessment/questionAnswer/' +
                        quizAnswer._id +
                        '/create/',
                    data,
                    { timeout: 5000 }
